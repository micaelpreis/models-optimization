...
def reset():
	f = open("outputs/paths/paths_" + sys.argv[1] + ".txt", "w")
	f.close()
...
def save_path(n):
	...
	f = open("outputs/paths/paths_" + sys.argv[1] + ".txt", "a")
	path = "".join(path)
	f.write("--- Path ---\n")
	f.write(path)
	f.close()
...
while True:
	# Get path
	subprocess.call(["glpsol","-m","paths.mod","-d","datafiles/"+sys.argv[1]+".dat","-y","output.dat"])
